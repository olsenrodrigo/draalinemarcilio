...
export default function FAQ() {
  ...
  return (
    <section id="faq" className="py-24" style={{ backgroundColor: "#EBF3EE" }}>
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        ...
        >
          <div className="inline-block px-4 py-2 rounded-full mb-6" style={{ backgroundColor: "rgba(46, 125, 82, 0.15)" }}>
            <span className="text-sm font-medium" style={{ color: "#1A3D2B" }}>Depoimentos</span>
          </div>
        ...
        <div className="grid md:grid-cols-3 gap-8 max-w-6xl mx-auto">
          {testimonials.map((item, index) => (
            ...
            >
              <Quote className="w-8 h-8 mb-4 opacity-20" style={{ color: "#2E7D52" }} />
              ...
              <div className="flex items-center gap-1 mb-3">
                {[1, 2, 3, 4, 5].map((star) => (
                  <Star key={star} className="w-4 h-4 fill-current" style={{ color: "#2E7D52" }} />
                ))}
              </div>

              <div>
                <p className="font-semibold text-sm" style={{ color: "#212529" }}>{item.name}</p>
                <p className="text-xs" style={{ color: "#2E7D52" }}>{item.detail}</p>
              </div>
            </motion.div>
          ))}
        </div>
        ...
        >
          ...
          <div className="flex flex-wrap justify-center gap-4 mb-8 text-lg" style={{ color: "rgba(235,243,238,0.9)" }}>
